...
const NavBar = ({ setMenuOpen }: NavBarProps) => {
  ...
  return (
    <motion.nav
      ...
      className={`justify-between shadow-2xl h-full flex w-full`}>

      <Link href="/">
        <Image src="/logo.png" alt="Logo de Rafaela Mazieiro" width={211} height={96} quality={100} unoptimized
          className="m-2 hover:opacity-50 brightness-110" />
      </Link>
      ...
      <motion.div onClick={toggleMenu}
        ...
        <div className={isOpen ? "hidden" : "bg-[#DC928F] brightness-110 h-[3px] group-hover:-translate-y-[2px]"}></div>
        <div className={isOpen ? "hidden" : "bg-[#DC928F] brightness-110 h-[3px]"}></div>
        <div className={isOpen ? "hidden" : "bg-[#DC928F] brightness-110 h-[3px] group-hover:translate-y-[2px]"}></div>
